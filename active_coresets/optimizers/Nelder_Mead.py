...
def nelder_mead(func, x_start, delta=1.7, tol=10e-6, no_improv_break=15,
                max_iter=5000, print_info=2):
    '''
    '''
    ###Initialize###
    ndim = len(x_start)
    ...
    while True:
        ...
        if print_info == 2:
            print('Current iteration: {0}, Num function calls: {1}, best so far: {2:.7f}'.format(niter, nfunc, best))
...
def minimizeEnergyObjective(f, init_params, num_params, delta, tol,
                            no_improv_break, max_iter, print_info):
    '''
